...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < file_names.size(); i++) {
        ...
        if (img.empty()) {
            std::cerr << "Failed to read image: " << file_names[i] << std::endl;
            continue;
        }
        ...
        cuda_preprocess(img.ptr(), img.cols, img.rows, device_buffers[0], kInputW, kInputH, stream);
        CUDA_CHECK(cudaStreamSynchronize(stream));
        ...
        //     for (size_t l = 0; l < res_batch[k].size(); l++) {
        //         std::cout << "image: " << file_names[i] << ", bbox: " << res_batch[k][l].bbox[0] << ", "
        //                   << res_batch[k][l].bbox[1] << ", " << res_batch[k][l].bbox[2] << ", "
        //                   << res_batch[k][l].bbox[3] << ", conf: " << res_batch[k][l].conf
        //                   << ", class_id: " << res_batch[k][l].class_id << std::endl;
        //     }
        // }
